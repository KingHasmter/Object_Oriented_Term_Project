...
public class LOM_inform extends JFrame {
	...
	public LOM_inform() {
		...
		//gbc.insets = new Insets(20, 20, 0, 0); // 왼쪽 여백 설정

		// 이미지 로드 및 표시
		Image image = new ImageIcon("img/apple_templete.png").getImage().getScaledInstance(480, 270, Image.SCALE_DEFAULT); // 이미지 크기 조정
		...
		mkabilityLabel = new JLabel("Marketability");
		mkabilityLabel.setFont(new Font(mkabilityLabel.getFont().getName(), Font.PLAIN, 20));
		panel.add(mkabilityLabel, gbc);
		
		
		// LABEL2 추가
		gbc.gridy = 1;
		resultLabel = new JLabel("Result");
		resultLabel.setFont(new Font(mkabilityLabel.getFont().getName(), Font.PLAIN, 20));
