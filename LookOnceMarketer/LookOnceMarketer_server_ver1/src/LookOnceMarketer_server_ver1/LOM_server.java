...
//input(client) -> 이미지 파일
//input(db) -> 전체 상품성 지수 배열, 전체 상품성 값 배열 (float array, int array)

//output(db) -> 상품성 지수, 상품성 값(float, int)
//output(client) -> 상품성 지수, 상품성 값(float, int)
		...
		try {
			...
			while (true) {
				if ((img = (Image) ImageIO.read(in)) != null) {
					...
					//1. marketability 계산 로직
					
					//2. 객체에 결과 값 저장
					
					//3. 결과 객체 socket 전송
					
					//4. chart update (addValue)
					
					//4. 결과 객체 DB insert
					
					//5. db fetch
					
					//6. mean, std, max, min 계산
					
					//7. 계산 결과 값 setText
					
					//8. detect 결과 이미지 setImg
